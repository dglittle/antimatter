...
antimatter.create = (self, send) => {
    ...
    if (!self.id) self.id = Math.random().toString(36).slice(2)
    if (!self.next_seq) self.next_seq = 0
    ...
    self.disconnect = peer => {
        self.receive({cmd: 'disconnect', peer})
    }
    ...
    self.receive = ({cmd, version, parents, patches, fissure, versions, fissures, unack_boundary, min_leaves, peer, conn}) => {
        if (cmd == 'get' || cmd == 'get_back') {
            ...
            if (ancestors(self.unack_boundary)[version]) return
            if (ancestors(self.acked_boundary)[version]) return
            ...
            let ack_versions = ancestors(self.acked_boundary)
            ...
            var our_conn_versions = ancestors(self.T, self.unack_boundary)
            var new_conn_versions = ancestors(self.T, unack_boundary)
            ...
            var ack_versions = ancestors(self.acked_boundary)
    ...
    function prune() {
        ...
        var acked = ancestors(self.T, self.acked_boundary)
    ...
    function add_version(version, parents, patches, sort_keys, is_anc) {
        ...
        function resolve_path(parse) {
            ...
            if (!cur || typeof(cur) != 'object' || cur.t == 'lit')
                cur = self.S = {t: 'val', S: create_space_dag_node(null, [cur])}
            var prev_S = null
            var prev_i = 0
...
if (typeof module != 'undefined') module.exports = loom
